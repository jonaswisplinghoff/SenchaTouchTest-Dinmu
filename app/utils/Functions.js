Ext.define('testApp.utils.Functions', {
    singleton: true,
	...
	See testApp.utils.Functions.getWeather(city + " " + country);. 
			...
			callback: function(records, operation, success) {
				...
				if (records.length > 0) {
					...
					if (city && country) {
						...
						testApp.utils.Functions.getWeather(city + " " + country);
					} else {
	...
	getCurrentLocation: function() {
		...
		Ext.device.Geolocation.getCurrentPosition({
			...
			success: function(position) {
				...
				//console.log(place);
				testApp.utils.Functions.getWeather(place);
				Ext.Viewport.unmask();
			},
			failure: function() {
				Ext.Viewport.unmask();
	...
	*/
	getWeather: function(place) {
		Ext.data.JsonP.request({
			...
			params: {
				key: testApp.utils.Functions.API_KEY,
				q: place,
				format: 'json',
				num_of_days: 1
			},
			success: function(result, request) {
				...
				try {
					...
					testApp.utils.Functions.createTemplate(weather[0]);
